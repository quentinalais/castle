...
async function prixdate(mois,uri) 
{
	...
	const response = await fetch("https://www.relaischateaux.com/fr/popin/availability/check?month=2019-"+mois+"&idEntity=22684%7C%7CSTD1DG&pax=2&room=1",
{
	...
	for(var j in liste_fd)
	{
		...
		ligne+=JSON.stringify({prix:[liste_finale[0][j],liste_finale[1][j]]})
		ligne+=","
	}
	console.log("taille liste 0 :"+liste_finale[0].length)
	console.log("taille liste 1 :"+liste_finale[1].length)
	ligne+="}"
	//console.log(ligne)
	return ligne
...
console.log(prixdate(mois,uri_chateau));


//console.log("taille liste 1"+abon[1].length)
